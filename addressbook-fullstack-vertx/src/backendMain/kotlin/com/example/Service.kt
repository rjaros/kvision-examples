...
suspend fun <RESP> WithContext.withProfile(block: suspend (Profile) -> RESP): RESP {
    val profile = (this.rctx.user() as MyUser).profile
    return block(profile)
}

actual class AddressService @Inject constructor(override val rctx: RoutingContext) : IAddressService, WithContext {
    ...
    override suspend fun deleteAddress(id: Int): Boolean = withProfile { profile ->
        dbQuery {
...
actual class ProfileService @Inject constructor(override val rctx: RoutingContext) : IProfileService, WithContext {
